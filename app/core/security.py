...
async def verify_password(plain_password: str | bytes, hashed_password: str | bytes) -> bool:
    """Сверяет полученный пароль и хешированный"""
    print(plain_password, "            ",hashed_password)
    return pwd_context.verify(plain_password, hashed_password)


async def authenticate_user(user_data: User) -> UserSchema:
...
async def get_user_from_token(token: str = Depends(get_token)):
    ...
    if expire is None or expire < datetime.utcnow().timestamp():
        raise HTTPException(status_code=401, detail=f"В jwt токене не указан exp либо истек срок действия токена")
    user_name: str = payload.get("sub")
    if not user_name:
        raise HTTPException(status_code=401, detail="В jwt токене не указан sub")
    user = await UsersDAO.find_one_or_none(name=user_name)
    if not user:
        raise HTTPException(status_code=401, detail="Пользователь не найдены")
    return user